...
<\body>
  ...
  Pure is known to work on Linux, FreeBSD, NetBSD, Mac OSX and MS Windows,
  ...
  Windows (AMD/Intel x86, 32 bit). There are a few known system-specific
  quirks and corresponding workarounds which are discussed below.
  ...
  compiling LLVM \<less\>=2.5 on 32 bit systems.

  Some older LLVM 2.5 packages for Linux are broken on x86-32 for this reason
  (this has been reported for Ubuntu 9.04 and Fedora Core 10), the symptom
  being that the Pure interpreter fails a lot of checks and/or segfaults
  right at startup. In that case you'll have to find a newer, corrected
  package or build your own LLVM from source instead.

  <subsubsection|64 Bit Systems>
  ...
  requires that you configure LLVM with <verbatim|--enable-pic> so that the
  static LLVM libraries can be linked into the runtime library. With LLVM 2.6
  and later, this option isn't needed anymore.
  ...
  Also note that with at least some versions of the Apple gcc compiler, with
  all warnings turned on you may get the (bogus) warning ``control reaches
  end of non-void function'' a couple of times in interpreter.cc. These are
  due to a bug in older gcc versions (see
  <hlink|http://gcc.gnu.org/bugzilla/show_bug.cgi?id=16558|http://gcc.gnu.org/bugzilla/show-bug.cgi?id=16558>),
  but they are harmless and can be ignored. These warnings should also go
  away once Apple upgrades its SDK to a newer gcc version.
