...
<\body>
  ...
  <\itemize>
    ...
    -b, and a new -f (a.k.a. \Ufont-size) option was added to change the font
    size of the GUI elements.

    <item>For compatibility with pd-faust, the -s (a.k.a. \Uslider-nums) flag
    is enabled by default now. The corresponding option is still supported
    for backward compatibility, and for overriding the new \Uno-slider-nums
  ...
  faster, which is a big advantage because most xml and json files don't take
  ...
  additional meta data in the Faust source (the XML format doesn't offer this
  in the latest Faust versions any more), and the capability to read dsp
  descriptions straight from running dsp instances in httpd-enabled Faust
  applications and Grame's new FaustLive environment. However, this format is
  ...
  At the time of this writing, these versions are only available in the Faust
  git repository.
  ...
  \Pin-place\Q, without installing anything except the required software as
  ...
  named \Pfaust2pd\Q which allows you to create Pd abstractions as
  \Pwrappers\Q around Faust units. The wrappers generated by faust2pd can be
  used in Pd patches just like any other Pd objects. They are much easier to
  operate than the \Pnaked\Q Faust plugins themselves, as they also provide
  \Pgraph-on-parent\Q GUI elements to inspect and change the control values.
  ...
  As of version 2.11, faust2pd can also process dsp descriptions in Faust's
  ...
  <\itemize>
    ...
    <item>\PAnonymous\Q groups and controls (groups and controls which have
  ...
  \Pbypass\Q or \Pmute\Q controls in the Faust source. However, if the
  ...
  <\itemize>
    ...
    operating \Pbare\Q Faust plugins in Pd. You'll rarely have to do this
  ...
  <verbatim|faust2pd> <verbatim|filename.dsp.json> instead. Please note that
  this format is only supported in the latest Faust versions (Faust 0.9.70
  and later).
  ...
  <verbatim|faust2pd> <slink|http://localhost:5510>. You can find out about
  ...
  <verbatim|faust2pd> <slink|http://localhost:7777/availableInterfaces>. This
  ...
  -h, \Uhelp display a short help message and exit -V, \Uversion display the
  version number and exit -b, \Ufake-buttons <nbsp> replace buttons (bangs)
  with checkboxes (toggles) -f, \Ufont-size <nbsp> font size for GUI elements
  (10 by default) -n, \Unvoices create a synth patch with the given number of
  voices -o, \Uoutput-file <nbsp> output file name (.pd file) -r,
  \Uradio-sliders <nbsp> radio controls for sliders -s, \Uslider-nums <nbsp>
  sliders with additional number control -X, \Uexclude exclude controls
  matching the given glob patterns -x, \Uwidth maximum width of the GUI area
  -y, \Uheight maximum height of the GUI area Just like the Faust plugin
  ...
  This generally works very well, but the control GUIs generated by faust2pd
  are somewhat hampered by the limited range of GUI elements available in a
  vanilla Pd installation. As a remedy, faust2pd provides various options to
  change the content of the generated wrapper and work around these
  limitations.
  ...
  <\itemize>
    ...
    controls, as these are implemented using sliders, see below. As of
    faust2pd 2.12, this option is actually the default now, but you can still
    disable it with the \Uno-slider-nums option.)
    ...
    OTOH, Faust doesn't support the \Pradio\Q control elements which Pd
    ...
    <item>Since Pd has no \Ptabbed\Q (notebook-like) GUI element, Faust's
  ...
  above can also be specified in the Faust source, as special meta data in
  the label of the toplevel group of the dsp. These take the form
  <verbatim|[pd:option]> or <verbatim|[pd:option=value]> where
  <verbatim|option> is any of the layout options understood by faust2pd. For
  instance:
  ...
  Alternatively, as of faust2pd 2.11, these options can also be specified as
  a single chunk of global meta data, using the <verbatim|faust2pd> key. So
  ...
  options. Also note that all this works with the JSON format only, as meta
  data in the Faust source isn't recorded in the XML format produced by
  recent Faust versions any more.
  ...
  faustxml><label|appendix-faustxml><label|appendix-faustxml>
  ...
  Note: As of faust2pd 2.11, the <hlink|<with|font-family|tt|info>|#faustxml::info>
  ...
  instance (typically something like <slink|http://localhost:5510>). This
  ...
  <slink|http://localhost:7777/availableInterfaces> to retrieve the list of
